...
void Draw_Color(int i) { // 색상을 그리는 함수 The function that change color 
	if (i == RUN) {
...
/*
로봇 왼쪽 손을 그리는 함수 Funtion that draws left hand
*/
...
/*
로봇 오른쪽 손을 그리는 함수 right hand of robot
*/
...
/*
땅을 그리는 함수 Function making ground
땅은 커다란 구로 설정하였다. Ground is a sphere
땅은 시간에 따라 회전하여서 마치 로봇이 rotate sphere like moving
...
void Walk() {
	...
	L_Arm_x = sin(time) * 25;
	R_Arm_x = -L_Arm_x;
	R_Arm_y = -abs(int(sin(time) * 30 + 15));
	L_Arm_y = -abs(int(-sin(time) * 30 + 15));

	R_Leg_y = abs(int(-sin(time) * 15 - 10));
	L_Leg_y = abs(int(sin(time) * 15 - 10));
	R_Leg_x = sin(time) * 25;
	L_Leg_x = -R_Leg_x;
	...
	glRotatef(sin(time) * 8, 0, 1, 0);


	float i = 0;
	i = abs(int(sin(time)*0.04));
	glPushMatrix();
...
void Jap() {
	...
	R_Arm_y = -abs(int(cos(time2) * 80));  //우측팔뚝 각도조절(팔을 뻗는 움직임표현을위하여 어깨의 sin()함수와 반대인 cos()함수 사용)
	L_Arm_y = -abs(int(-cos(time2) * 80)); //좌측팔뚝 각도조절(팔을 뻗는 움직임표현을위하여 어깨의 sin()함수와 반대인 cos()함수 사용)

	R_Leg_y = abs(int(-sin(time) * 30 - 20));  //우측종아리 각도조절(abs절대값을 줌으로써 종아리가 앞으로 꺾이지 않는 한계점을 설정)
	L_Leg_y = abs(int(sin(time) * 30 - 20));   //좌측종아리 각도조절(abs절대값을 줌으로써 종아리가 앞으로 꺾이지 않는 한계점을 설정)
...
void Rocket() {
	...
	R = 2 * abs(int(-sin(time2)*0.2 - 0.2)) + 0.2;//우측 로켓 움직임 설정
												  //abs절대값으로 로켓이 앞쪽으로 나가게 설정.
												  //+0.2를 통해 로켓의 처음위치 조절. 2*를 통해 로켓이 나가는 거리조절.
												  //sin()함수를 통해 주기적인 움직임 설정.

	R2 = 2 * abs(int(sin(time2)*0.2 - 0.2)) + 0.2;//좌측 로켓 움직임 설정 
												  //abs절대값으로 로켓이 앞쪽으로 나가게 설정.
												  //+0.2를 통해 로켓의 처음위치 조절. 2*를 통해 로켓이 나가는 거리조절.
												  //sin()함수를 통해 주기적인 움직임 설정.
